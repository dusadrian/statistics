...
function(x, y, n1, n2) {
    ...
    if (sum(verificari) == 4) {
        if (sum(verificari[1:2]) == 0) {
            admisc::stopError(paste(erori[3:6][verificari[1:4]], collapse="\n       "), "")
        }
        else {
            admisc::stopError(paste(erori[1:2][verificari[1:2]], collapse="\n       "), "")
        }
    }
    else if (sum(verificari) == 3) {
        if (length(x) == 1) {
            admisc::stopError(paste(c(paste("If", x, "is the standard deviation for the first group, then:"),
                       erori[4:6]), collapse="\n       "), "")
        }
        else {
        ...
        if (missing(x) | missing(y)) {
            ...
            admisc::stopError(paste(erori[5:6], collapse="\n       "), "")
        }
        else if (any(length(x) == 1, length(y) == 1)) {
            admisc::stopError(erori[7:8][which(length(x) == 1, length(y) == 1)], "")
        }
        else {
            var1 <- var(x)
            n1 <- length(x)
